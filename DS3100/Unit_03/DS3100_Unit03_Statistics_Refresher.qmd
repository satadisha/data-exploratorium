...
- Cohen's `d`/`effect_size` standardizes the effect size in standard deviations: $d = \frac{\mu_1-\mu_0}{\sigma}$
- `type = one.sample`: tells `pwr.t.test()` that this is a <span style="color:#1565C0;">**one-sample t-test**</span>. In our example, we have one sample of patients after the new system, compared against a fixed reference mean of 30 minutes.
- `alternative = less`/`alternative = smaller`: specifies the direction of the alternative hypothesis. Our research question is whether the new system reduces waiting time and hence the relevant effect will be in the <span style="color:#1565C0;">left tail of the distribution</span>.
...
For the same effect size, increasing the sample size makes the estimates more precise and increases the probability of detecting the effect.
$$\text{Power} \propto \frac{\text{Effect Size} \times \sqrt{\text{Sample Size}}}{\text{Significance Level } (\alpha)}$$
...
|---|---|---|
| $H_0$ true (*no effect*) | Reject $H_0$ | <span style="color:#1565C0;">Type I error</span> or False Positive |
| $H_0$ false (*effect exists*) | Fail to reject $H_0$ | <span style="color:#1565C0;">Type II error</span> or False Negative|

:::

::: {.columns .v-center}

::: {.column width="52%" style="font-size:0.58em;"}

Think of two distributions: the null (where $H_0$ true) and an alternative (where $H_A$ is specified).

- $H_0$ distribution sets the rejection cutoff
- $H_A$ distribution asks how often we cross that cutoff

<span style="color:#1565C0;">Type I error rate</span> is tied to the significance level. It is area of the null distribution inside the rejection region. 
$\alpha=P(\text{reject }H_0\mid H_0\text{ true})$

<span style="color:#1565C0;">Type II error rate</span> ($\beta$) is controlled by Statistical Power. It is the area of the alternative distribution inside that same rejection region. $\beta=1-\text{Power}$

:::

::: {.column width="48%" style="font-size:0.58em;"}

![](images/statistical-power.png){width=100%}
...
- μ = population mean. The sampling distribution of $\bar X$ is centered at the population mean.
- $\sigma$ = population standard deviation, describes how much individual observations vary in the population.
- $\sigma^2$ = population variance.
- $n$ = sample size.

In other words, <span style="color:#1565C0;">upon standardization</span> $\frac{\bar{X}-\mu}{\frac{\sigma}{\sqrt{n}}} \approx N\left(0,1\right)$
...
- Even if your data is heavily skewed, binary, or completely uniform, the average of those values tend to form an **approximately bell-shaped distribution** if you collect enough samples.
- **Asymptotic Normality** (The Generalized CLT): Many other statistics used in inference— proportions, differences, regression coefficients, etc— have predictable sampling behavior. With sufficiently large samples, their sampling distributions can be approximated to familiar reference distributions, allowing us to calculate probabilities and $p$-values.

:::

---

## Why the normal curve appears so much in inferential processes
...
## Standardizing a sample statistic

::: {style="font-size:0.60em;"}

*Standardization* maps your sample statistic onto a unit-less scale.<br>
...
=
\frac{
\text{observed sample statistic} -
\text{value specified by }H_0
}{
\text{standard error}
}
$$

#### Example: the $z$ statistic for a sample mean
...
<span style="color:#1565C0;">Interpretation</span>: *How many standard errors is the observed sample mean from the mean specified by the null hypothesis?*

Under $H_0$, $z$ follows a <span style="color:#1565C0;">**standard normal distribution**</span> when the relevant
...
Our reference distribution therefore becomes the <span style="color:#1565C0;">**Student's t distribution**</span>.
...
- Is centered at 0.
- Unlike the standard normal distribution —which is fixed and never changes— the $t$-distribution is actually a family of curves. Its exact shape is determined entirely by its <span style="color:#1565C0;">**degrees of freedom**</span> ($df$).
- For small samples the $t$ distribution has heavier tails than the standard normal to account for higher uncertainty.
...
| Reference distribution | standard (*upon standardization*) normal | Student's t |
...
- the population SD is known or estimated
- sampling distribution of the statistic itself must be normal (centered around the true parameter with its specific standard error)
:::
...
A [standard $z$-table](https://statisticsbyjim.com/hypothesis-testing/z-table/) gives the **area to the left** of a given $z$-score. To find a value:

1. Use the **row** for the first two digits of $z$.
2. Use the **column** for the hundredths digit.
3. The cell gives $P(Z\le z)$.

**Example**: For $z=1.65$ look up row: $1.6$, and, column: $0.05$. 
The table gives: $P(Z\le1.65)\approx0.9505$. So about **95.05\%** of the standard normal distribution lies to the left of $z=1.65$.
...
<div style="position:relative; width:100%; height:430px;">
  ...
  <span style="color:#1565C0;"> **One-tailed test** </span> ($\alpha=0.05$): Only the tail in the direction of the alternative hypothesis is looked into. So you look for an area of $0.95$ or $0.05$, depending on which tail. 
  Looking into the table the closest to our target $\alpha$ is $0.04947$. This area is at the row and column intersection for the z-value of $-1.65$. That’s our critical value! Technically our $\alpha$ falls between the z-values of $-1.64$ and $-1.65$ but let's go with the marginally more conservative critical z-value of $-1.65$.


  <span style="color:#1565C0;"> **Two-tailed test** </span> ($\alpha=0.05$): Because the rejection region is split between **two tails**, first divide the significance level by 2: $\frac{\alpha}{2} = \frac{0.05}{2} = 0.025$. So each tail contains **2.5\%** of the null distribution. 
  If the table reports cumulative area to the left, look for: $1-0.025=0.975$ The corresponding critical value is approximately: $\boxed{z^*=1.96}$. Thus the rejection regions are: $z<-1.96 \text{ or } z>1.96$
...
- **Left tail:** use the table value directly.
- **Right tail:** $1-\text{table value}$.
- **Two tails:** calculate the appropriate tail and **double it** when the tails are symmetric.
...
### What statistical test fits your research question?
...
The tail choice changes the reference region and therefore the p-value.<br>
...
- [Standard $z$-table](https://statisticsbyjim.com/hypothesis-testing/z-table/)
