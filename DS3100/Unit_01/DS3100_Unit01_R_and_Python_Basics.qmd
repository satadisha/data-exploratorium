...
A data frame is a data structure designed to deal with **2-D tabular data**. Here rows represent observations, columns represent variables, and columns may have different types.<br>

- In R, data frames can be though of a special type of list where elements are vectors, each representing a column of the resulting data frame. Hence all column vectors at the time of creating a data frame should ideally have the same number of elements.

- In Python, dataframes are a custom object built using `NumPy` arrays.
...
The **tidyverse** is a collection of R packages designed to facilitate common operations in data analysis.

For this course, the most important pieces are:

| Package | What we use it for |
|---|---|
| `readr` | importing rectangular data |
| `dplyr` | manipulating and summarizing data |
| `tidyr` | restructuring data into tidy form |
| `ggplot2` | visualization |
| `tibble` | modern tabular data in R |
| `forcats` | categorical variables / factors |

:::

::: {.column width="50%" style="text-align:center;"}

![](images/tidyverse.png){width=90%}
...
We will look into several wrangling operations and tie them to specific `tidyverse` verbs. There are conceptually similar functions available in Python to answer analytical questions.

::: {.columns}

::: {.column width="45%" style="font-size:0.75em;"}
| Analytical question | `dplyr` verb |
|---|---|
| Selecting a subset of rows | `filter()` |
| Selecting columns | `select()` |
| Ordering observations | `arrange()` |
| Renaming variables | `rename()` |
| Creating/changing variables | `mutate()` |
| Creating summaries | `summarize()` |
| Creating groups | `group_by()` |
| Merging information from separate datasets | `*_join()` |
:::
...
Most data analyses roughly follow the workflow below.

1. **Define your research question.** Here we ask: <span style="color:#1565C0;">*How do educational achievement outcomes vary across school districts in Tennessee?*</span>

2. **Familiarize yourself with the dataset.** Understanding the **provenance and structure of the data** is essential before beginning an analysis.

3. **Investigate missingness** and how it might impact downstream calculations.

4. Define the **analysis population.**

5. **Create or transform variables as needed.**

6. Generate appropriate **summary statistics.**

7. **Ask whether one dataset is enough.** An analytical question may require information that is not contained in the  original table. In such cases, strategically combining multiple data sources may be necessary.

8. **Explore the analysis-ready data.** *(We will return to this during EDA.)* Summary statistics rarely tell the whole story. We need to find the patterns behind numbers.

9. **Visualize important patterns** that is difficult to see from summary statistics alone. *(We will return to this during visualization.)*

10. **Interpret and communicate** your findings.
...
2. `%>%` → pass that data frame to the next operation (*value of the left-hand side of the operator becomes the first argument to the right-hand side*).
...
- load your data;
- understand your data and its sources and find more information if needed;
- set a research question
- lay out an analysis workflow;
- carry out necessary wrangling operations.
