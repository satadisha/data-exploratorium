...
<span style="color:#1565C0; font-size:1.8em; font-weight:600;"> Data objects in R and Python</span>
...
These distinctions matter because they determine **what a data structure can contain** and **how we access and manipulate its elements**.
...
| List | `list` | ordered heterogeneous container |
...
```
<br>
<small>A useful sequence shortcut:</small>

```r
5:8
```
<br>
<small>This creates a vector of integers from 5 to 8.</small>
...
- In **R**, `5:8` produces `5, 6, 7, 8` — **both endpoints are included**.
- In **Python**, `range(5, 8)` produces `5, 6, 7` — the **stop value `8` is excluded**.

Therefore, the Python equivalent of R's `5:8` would be `range(5, 9)`.
:::
...
Python typically uses explicit labels through a tabular structure such as a pandas `DataFrame` rather than relying on matrix row/column names in the same way. That's the data structure you will see in the next section and more commonly use during large scale data analysis.
...
A data frame is a **2-D table** in which columns may have different types.
...
- Python lists can contain values and other containers as well.
:::
...
The broad concept is shared:

> **Select or extract a part of an object.**

The syntax depends on the object's structure.
...
1. Print the first few observations in `ten`.

2. Determine the **number of rows and columns** in the data frame.

3. Print the **column names**. How many different variables are available?
...
6. **Bonus**: How many columns in this dataset have **missing values**?
...
<small>Missing data require explicit attention.</small>
...
<small>Pandas commonly uses `NaN` / `NA`-like missing values.</small>
...
- Missingness can be informative and may require special handling.
...
<span style="color:#1565C0; font-size:1.8em; font-weight:600;"> A small end-to-end example</span>

---

## Pop Icons: start with a simple vector
...
pop_icons <- c(
  ...
  "Chris Strauss", "Olivia Rodrigo",
  "Alex Christensen", "Beyonce",
  "Britney Spears"
)
```
...
```r
pop_icons <- c(pop_icons, "Andres Gannon")
```

#### Python

```python
pop_icons.append("Andres Gannon")
```

Same conceptual operation:

> **Add one new observation to a collection.**
...
```r
pop_df <- data.frame(pop_icons = pop_icons)
```

#### Python

```python
pop_df = pd.DataFrame({"pop_icons": pop_icons})
```

Now the object is explicitly tabular, which makes row/column operations natural.

---

# Part 7

<span style="color:#1565C0; font-size:1.8em; font-weight:600;"> Activity: translate the workflow </span>

Choose **R or Python**.

::: {.callout-note style="font-size:0.75em;"}
Starting from a table called `df`:

1. Remove rows where `percent_below` is missing.
2. Keep observations where `overall_subject == "ELA"`.
3. Add the condition `percent_below >= 20`.
4. Rename `name` to `school_name`.
5. Create `diff = percent_below - percent_below_previous`.
6. Compute the mean of `diff`.
7. Group by `school_name` and calculate the mean of `percent_below`.
...
::: {.callout-tip}
**Bring your questions to lab.** The goal of the first lab is not to prove that you already know R or Python; it is to make sure you can work comfortably in the environment before we move deeper into data analysis.
:::
